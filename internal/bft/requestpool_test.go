	...
	t.Run("auto remove", func(t *testing.T) {
		...
		pool := bft.NewPool(log, insp, timeoutHandler,
			bft.PoolOptions{
				...
				ForwardTimeout:    10 * time.Millisecond,
				ComplainTimeout:   10 * time.Millisecond,
				AutoRemoveTimeout: 100 * time.Millisecond,
			},
			submittedChan,
		)
		defer pool.Close()
...
func TestReqPoolTimesOnlyTheHead(t *testing.T) {
	basicLog, err := zap.NewDevelopment()
	assert.NoError(t, err)
	log := basicLog.Sugar()
	...
	timeoutHandler := &mocks.RequestTimeoutHandler{}
	req1Forwarded := make(chan struct{})
	req2Forwarded := make(chan struct{})
	timeoutHandler.On("OnRequestTimeout", byteReq1, insp.RequestID(byteReq1)).Run(func(mock.Arguments) {
		close(req1Forwarded)
	}).Return()
	timeoutHandler.On("OnRequestTimeout", byteReq2, insp.RequestID(byteReq2)).Run(func(mock.Arguments) {
		close(req2Forwarded)
	}).Return()
	...
	<-req1Forwarded
	// Both requests were submitted together, so a per-request timer on the
	// second one would have fired by now.
	select {
	case <-req2Forwarded:
		t.Fatal("second request was timed while it was not at the head of the pool")
	case <-time.After(100 * time.Millisecond):
	}
	timeoutHandler.AssertNumberOfCalls(t, "OnRequestTimeout", 1)

	assert.NoError(t, pool.RemoveRequest(insp.RequestID(byteReq1)))
	select {
	case <-req2Forwarded:
	case <-time.After(time.Second):
		t.Fatal("second request was not timed after it became the head of the pool")
	}
	timeoutHandler.AssertNumberOfCalls(t, "OnRequestTimeout", 2)
}

func TestReqPoolAutoRemoveAgeIsNotResetByRestart(t *testing.T) {
	basicLog, err := zap.NewDevelopment()
	assert.NoError(t, err)
	log := basicLog.Sugar()
	...
	timeoutHandler.On("OnLeaderFwdRequestTimeout", byteReq1, insp.RequestID(byteReq1)).Run(func(mock.Arguments) {
		assert.Fail(t, "called OnLeaderFwdRequestTimeout")
	}).Return()
	removed := make(chan struct{})
	timeoutHandler.On("OnAutoRemoveTimeout", insp.RequestID(byteReq1)).Run(func(mock.Arguments) {
		close(removed)
	}).Return()
	...
	// Keep restarting past the age limit: a restart gives the head a fresh
	// forward budget but must not extend its age limit.
	...
	case <-removed:
	case <-time.After(time.Second):
		t.Fatal("request was not auto-removed at its age limit")
	}
	assert.Equal(t, 0, pool.Size())
}

func TestReqPoolAutoRemoveDrainsOverdueBacklog(t *testing.T) {
	basicLog, err := zap.NewDevelopment()
	assert.NoError(t, err)
	log := basicLog.Sugar()

	requests := [][]byte{
		makeTestRequest("1", "1", "foo"),
		makeTestRequest("2", "2", "foo"),
		makeTestRequest("3", "3", "foo"),
	}
	insp := &testRequestInspector{}

	timeoutHandler := &mocks.RequestTimeoutHandler{}
	removedWG := &sync.WaitGroup{}
	removedWG.Add(len(requests))
	for _, req := range requests {
		timeoutHandler.On("OnAutoRemoveTimeout", insp.RequestID(req)).Run(func(mock.Arguments) {
			removedWG.Done()
		}).Return()
	}
	...
	pool := bft.NewPool(log, insp, timeoutHandler,
		bft.PoolOptions{
			...
			ForwardTimeout:    time.Hour,
			ComplainTimeout:   time.Hour,
			AutoRemoveTimeout: 50 * time.Millisecond,
		},
		nil,
	)
	defer pool.Close()
	...
	for _, req := range requests {
		assert.NoError(t, pool.Submit(req))
	}

	// Only the head is timed, but once it is collected the next head is
	// already past its age limit and must go immediately rather than after
	// another full interval.
	allRemoved := make(chan struct{})
	go func() {
		removedWG.Wait()
		close(allRemoved)
	}()
	...
	case <-time.After(2 * time.Second):
		t.Fatal("overdue requests were not drained")
	}
	assert.Equal(t, 0, pool.Size())
}
