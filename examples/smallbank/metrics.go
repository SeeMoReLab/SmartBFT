...
const maxLatencyMS = int64(10000)
...
func newBenchmarkMetrics() *benchmarkMetrics {
	return &benchmarkMetrics{histogram: make(map[int64]int64)}
}
...
func (m *benchmarkMetrics) record(success bool, latency time.Duration) {
	if success {
		...
		bucket := min(int64(latency/time.Millisecond), maxLatencyMS)
		m.lock.Lock()
		m.histogram[bucket]++
		m.lock.Unlock()
		return
...
func printResults(label string, duration time.Duration, snap metricsSnapshot) {
	...
	p50 := percentile(snap.histogram, snap.success, 0.50)
	p95 := percentile(snap.histogram, snap.success, 0.95)
	p99 := percentile(snap.histogram, snap.success, 0.99)

	if label == "Monitor" {
		fmt.Printf("%s duration=%.3fs trxs=%d succ=%d err=%d tps=%.3f avg_ms=%.3f p50=%d p95=%d p99=%d\n",
			label, seconds, total, snap.success, snap.errors, tps, avgMS, p50, p95, p99)
		return
	}

	fmt.Printf("======================================================================\n")
	fmt.Printf("%s results:\n", label)
	fmt.Printf("duration=%.3fs trxs=%d succ=%d err=%d tps=%.3f avg_ms=%.3f p50=%d p95=%d p99=%d\n",
		seconds, total, snap.success, snap.errors, tps, avgMS, p50, p95, p99)
	fmt.Printf("======================================================================\n")
}

func percentile(hist map[int64]int64, successes int64, pct float64) int64 {
	if successes <= 0 {
		return 0
	}
